...
const GLOSSARY: Entry[] = [
  ...
  { term: 'Offline-first', short: 'Software that works fully without an internet connection.', body: 'An offline-first app queues every write locally, syncs when online, and resolves conflicts automatically. Calendro\'s mobile app (shipping Q4 2026) is being built this way so you can schedule on airplanes and in subways.', related: ['CRDT'] },
  { term: 'OAuth 2.0', short: 'The standard protocol for delegating access without sharing passwords.', body: 'When you click "Sign in with Google," OAuth is what\'s happening. Calendro uses OAuth to request specific scopes (just calendar read/write) without ever seeing your Google password.' },
  ...
  { term: 'RFC 5545', short: 'The IETF specification for iCalendar.', body: 'Published in 2009 and updated since, RFC 5545 is the authoritative technical standard for calendar data. When Calendro generates or parses .ics files, we\'re following this spec line by line. It is, famously, among the more difficult RFCs.' },
  { term: 'RRULE', short: 'The iCalendar field that defines how an event repeats.', body: 'RRULE is a compact string that encodes recurrence patterns. "FREQ=WEEKLY;BYDAY=MO,WE,FR" means "every Monday, Wednesday, and Friday." Calendro\'s parser can generate RRULEs from phrases like "every weekday" or "first Monday of each month."', related: ['Recurring event'] },
  { term: 'Smart time suggestions', short: 'AI that proposes a meeting slot based on your existing calendar.', body: 'When you type an event without a time, Calendro\'s smart-scheduling engine finds a free slot that respects your existing meetings, focus windows, working hours, and timezone. It runs locally for speed and privacy.' },
  { term: 'SSO', short: 'Single Sign-On — one login for many apps.', body: 'SSO lets users sign in to Calendro using their organization\'s identity provider (Okta, Azure AD, Google Workspace). Calendro supports SAML 2.0 and OIDC on the Enterprise tier.' },
  { term: 'Timezone', short: 'A region that shares a standard time offset from UTC.', body: 'Timezones are stored in the IANA database (e.g., "America/New_York") and are distinct from static UTC offsets because they include daylight-saving rules. Calendro stores every event in a timezone; display is user-configurable.', related: ['TZID', 'UTC'] },
  ...
  { term: 'UTC', short: 'Coordinated Universal Time — the reference clock for the globe.', body: 'UTC is the timezone-less timekeeping standard. Internally, Calendro converts everything to UTC for storage and comparison, then back to the user\'s display timezone at render time.', related: ['Timezone'] },
  { term: 'Vision model', short: 'An AI model that reads images, not just text.', body: 'Calendro\'s photo-to-events feature is powered by Gemini\'s vision capabilities. The model reads schedules from photos — exam timetables, class schedules, concert bills — and extracts every event in one call.' },
  { term: 'Webhooks', short: 'HTTP callbacks that notify your server when something happens.', body: 'Instead of polling every minute to check for new events, webhook-enabled calendars push a small HTTP POST to Calendro within seconds of any change. We support webhooks both inbound (from Google) and outbound (to your API).' },
  { term: 'Zod', short: 'A TypeScript-first schema validation library.', body: 'Zod is used throughout Calendro to validate data — especially the JSON Gemini returns from parsing requests. If Gemini\'s output doesn\'t match the expected schema, we fall back to chrono-node\'s result. This is how we ship AI safely.', related: ['Gemini'] },
]
  ...
  return (
    <>
      ...
      <CTA
        ...
        subtitle="Try Calendro for free — type a sentence and watch the parser do its thing."
