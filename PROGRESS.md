...
### Iteration 25
- **Task:** Build `/security/report` — a responsible-disclosure vulnerability reporting page.
- **Status:** Completed
- **Summary:** Built `app/(marketing)/security/report/page.tsx` with a disclosure-v2.1 badge in the hero, a 3-channel contact grid (PGP email marked Recommended, encrypted form, HackerOne), a 2-column In-Scope / Out-of-Scope section with emerald/rose accents (8 items each), a 5-row commitments timeline (1 day ack, 3 days assessment, 14 days plan, 90 days fix, post-fix credit) with a business-day note, a 4-item Safe-Harbor panel with individual shield-accent cards, a 6-entry Hall-of-Fame table with year/researcher/finding/severity columns (tonally colored severity pill), and a PGP-key block with fingerprint + download link. Also updated the `/security` hero button to route here instead of mailto. Linked from header (Company), footer (Legal), HTML sitemap, and sitemap.xml. Build clean.

### Iteration 26
- **Task:** Build `/ambassadors` — a capped, no-payout creator program distinct from /affiliates.
- **Status:** Completed
- **Summary:** Added `lib/ambassadors.ts` with 8 current ambassadors (Mei Lin, Tomás Levi, Hana Kobayashi, Pauline Rivera, Isabelle Moreau, Jonas Berg, Vincent Du, Priya Shankar) — each with name, role, location, monogram initials, quote, link to related content on the site, and since-year — plus 6 on-purpose perks (lifetime Pro, launch-week beta, annual crafted merch, founder dinner, Wall-of-Love profile, direct channel), 3 picking principles (small by design, taste not reach, no quotas), a 4-item "what we ask" list, and 6 pieces of ambassador-produced content (essays, talks, podcasts, newsletters) with kind + by + where + at. Built `app/(marketing)/ambassadors/page.tsx` with: hero; a cohort-status bar with filled/cap numbers and a gold progress fill; a 6-card perks grid; an 8-card current-cohort grid with monogram avatars + quotes + deep-links; a 3-card principles ordered list; a 2-column What-We-Ask / What-We-Will-Never-Ask section with emerald/rose accents; a divider list of ambassador content with per-kind icon; and a 4-Q FAQ. Linked from header (Company), footer (Company), HTML sitemap, and sitemap.xml. Build clean.
